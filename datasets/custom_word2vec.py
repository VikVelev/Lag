...
class SimpleWord2VecEmbedder:
    ...
    Highly optimized with vectorization, pre-computed negative sampling, and inlining.
    """
    ...
    def _build_vocab_and_corpus(self, sentences: List[List[str]]) -> None:
        ...
        if self.vocab_size > 0:
            ...
            freqs = np.array(
                [vocab[self.idx_to_word[i]] for i in range(self.vocab_size)]
            )
            ...
            keep_probs = (np.sqrt(word_fractions / self.subsample_threshold) + 1) * (
                self.subsample_threshold / word_fractions
            )
            ...
            for sentence in sentences:
                ...
                if (
                    len(int_sentence) > 1
                ):  # Only keep sentences with at least 2 words left
                    self.compiled_corpus.append(np.array(int_sentence, dtype=np.int32))

            print(
                f"Subsampling dropped {words_dropped} highly frequent words. Kept {words_kept}."
            )
    ...
    def train(self, sentences: List[List[str]]) -> None:
        ...
        for epoch in range(self.epochs):
            ...
            for indices in self.compiled_corpus:
                ...
                for i, center_idx in enumerate(indices):
                    ...
                    for j in range(start, end):
                        ...
                        neg_indices = self.neg_table[
                            self.neg_pointer : self.neg_pointer + self.negative_samples
                        ]
                        self.neg_pointer += self.negative_samples
            ...
            print(
                f"Epoch {epoch + 1}/{self.epochs}, Loss: {avg_loss:.4f}, LR: {current_lr:.5f}"
            )
...
if __name__ == "__main__":
    ...
    if corpus_sentences:
        embedder = SimpleWord2VecEmbedder(
            vector_size=50, window=6, min_count=2, epochs=150
        )
